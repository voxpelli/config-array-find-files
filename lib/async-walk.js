 ...
 * @property {Dirent} dirent The directory entry.
 */

/**
 * @typedef {object} AsyncWalkOptions
 * @property {string} basePath The directory to walk.
 * @property {(entry: WalkEntry) => boolean | Promise<boolean>} [deepFilter] Filter for directory traversal. Defaults to always true.
 * @property {(entry: WalkEntry) => boolean | Promise<boolean>} [entryFilter] Filter for file inclusion. Defaults to always true.
 * @property {boolean} [followSymbolicLinks] Whether to follow symbolic links.
 * @property {AbortSignal | undefined} [signal] An AbortSignal to cancel the traversal.
 * @property {((error: NodeJS.ErrnoException) => boolean) | undefined} [errorFilter] Optional function to filter errors. Return true to skip the error.
 */
...
export async function asyncWalk (options) {
  const { basePath, deepFilter = () => true, entryFilter = () => true, errorFilter, followSymbolicLinks, signal } = options;
  /** @type {string[]} */
  const results = [];
